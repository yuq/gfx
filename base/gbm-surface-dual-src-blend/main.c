...
void Render(void)
{
	...
	/* fb_dst = out1 * out0 + 1 * fb_dst */
	glBlendEquationSeparate(GL_FUNC_ADD, GL_FUNC_ADD);
	glBlendFuncSeparate(GL_SRC1_COLOR, GL_ONE, GL_SRC1_ALPHA, GL_ONE);
	...
	for (int i = 0; i < MRT_NUM; i++) {
		...
		char name[32];
		snprintf(name, sizeof(name), "screenshot.%d.png", i);
