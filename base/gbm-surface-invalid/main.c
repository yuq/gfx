...
static void render(void)
{
	...
	load_uniform_buffer("global.raw", 0, 2512);
        load_uniform_buffer("per_model.raw", 1, 256);
	...
	fill_buffer("data1.raw", data_map, 0, data_max_size);
	fill_buffer("data2.raw", data_map, 0x1ad000, data_max_size);
	fill_buffer("data3.raw", data_map, 0x1c4000, data_max_size);
	fill_buffer("data4.raw", data_map, 0x20a000, data_max_size);

	glBindBuffer(GL_COPY_READ_BUFFER, data_buffer);

	GLuint vertex_buffer = create_buffer(GL_ARRAY_BUFFER, 1410240, 728748);
	GLuint element_buffer = create_buffer(GL_ELEMENT_ARRAY_BUFFER, 1356080, 54160);
	GLuint per_pid_buffer = create_buffer(GL_UNIFORM_BUFFER, 2138994, 7680);
	...
	glBindBufferRange(GL_UNIFORM_BUFFER, 2, per_pid_buffer, 3840, 256);

	glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
	assert(glGetError() == GL_NO_ERROR);

	const int num_elements = 2392;
#ifdef FEEDBACK
	const int tbo_size = num_elements * 16;
	GLuint tbo;
	glGenBuffers(1, &tbo);
	...
	glDrawElements(GL_LINES, num_elements, GL_UNSIGNED_INT, NULL);
	assert(glGetError() == GL_NO_ERROR);
...
static void render_feedback(const char *name)
{
	...
	const int num_vertex = feed_size / 16;
	printf("feed vertex %d\n", num_vertex);

	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 4, GL_FLOAT, 0, 16, feed);	
