...
bool use_pipeline_shader = false;
...
{
	GLuint shader[2];
	assert((shader[0] = LoadShader("vert.glsl", GL_VERTEX_SHADER)) != 0);
	assert((shader[1] = LoadShader("frag.glsl", GL_FRAGMENT_SHADER)) != 0);

	if (use_pipeline_shader) {
		GLuint program[2];
		for (int i = 0; i < 2; i++) {
			program[i] = glCreateProgram();
			glAttachShader(program[i], shader[i]);
			glProgramParameteri(program[i], GL_PROGRAM_SEPARABLE, 1);
			LinkProgram(program[i]);
		}

		GLuint pipeline;
		glGenProgramPipelines(1, &pipeline);

		glUseProgramStages(pipeline, GL_VERTEX_SHADER_BIT, program[0]);
		glUseProgramStages(pipeline, GL_FRAGMENT_SHADER_BIT, program[1]);
		...
		if (!status) {
			GLint len = 0;
			glGetProgramPipelineiv(pipeline, GL_INFO_LOG_LENGTH, &len);
			if (len > 1) {
				char *log = malloc(len);
				glGetProgramPipelineInfoLog(pipeline, len, NULL, log);
				fprintf(stderr, "Error validate program pipeline:\n%s\n", log);
				free(log);
			}
			exit(1);
		}
