...
#if defined(SHADER_FRINGE_NODE_BOTTOM) && !defined(SHADER_FRINGE_NODE_TOP) 
#error "FEATURE ERROR: FRINGE_NODE_BOTTOM without FRINGE_NODE_TOP"
#endif
#if defined(SHADER_USE_NODAL_NORMALS) && defined(SHADER_SMOOTH_NORMALS) 
#error "FEATURE ERROR: USE_NODAL_NORMALS and SMOOTH_NORMALS"
#endif
...
layout(location = SHADER_3DMODEL33_POSITION_ATTRIB_LOC) in vec3 position;
#if defined(SHADER_FRINGE_NODE_TOP)
layout(location = SHADER_3DMODEL33_FRINGE_NODE_TOP_ATTRIB_LOC) in float fringe_node_top;
#endif
#if defined(SHADER_FRINGE_NODE_BOTTOM)
layout(location = SHADER_3DMODEL33_FRINGE_NODE_BOTTOM_ATTRIB_LOC) in float fringe_node_bottom;
#endif
#if defined(SHADER_USE_BLUR)
layout(location = SHADER_3DMODEL33_VELOCITY_COLOR_ATTRIB_LOC) in vec3 velocity_color;
#endif
#if defined(SHADER_LIC_OVERLAY) || defined(SHADER_LIC_SCREEN_SPACE)
layout(location = SHADER_3DMODEL33_LIC_VECTORS_ATTRIB_LOC) in vec3 vectors;
#endif
#if defined(SHADER_USE_TEX_COORDS)
layout(location = SHADER_3DMODEL33_TEX_COORDS_ATTRIB_LOC) in vec2 tex_coords;
#endif
#if defined(SHADER_USE_NODAL_NORMALS)
layout(location = SHADER_3DMODEL33_NODAL_NORMALS_ATTRIB_LOC) in uint nodal_normals;
#endif
#if defined(SHADER_EXTRA_ELEMENTS) && defined(SHADER_THICK_SHELLS)
layout(location = SHADER_3DMODEL33_EXTRA_THICKNESS_ATTRIB_LOC) in vec2 thickness_values;
layout(location = SHADER_3DMODEL33_EXTRA_THICKNESS_INTERNAL_EDGE_MASK_ATTRIB_LOC) in uint thickness_internal_edge_mask;
#endif
#if defined(SHADER_USE_NODAL_NORMALS_3F)
layout(location = SHADER_3DMODEL33_NODAL_NORMALS_3F_ATTRIB_LOC) in vec3 nodal_normals;
#endif
#if defined(SHADER_THICK_SHELLS_GET_NODAL_THICKNESS)
layout(location = SHADER_3DMODEL33_NODAL_THICKNESS_ATTRIB_LOC) in float nodal_thickness;
#endif
#if defined(SHADER_USE_NODAL_COLOR)
layout(location = SHADER_3DMODEL33_NODAL_COLOR_ATTRIB_LOC) in vec4 nodal_color;
#endif
#if defined(SHADER_COLOR_VARIANCE) && defined(SHADER_EXTRA_ELEMENTS)
layout(location = SHADER_3DMODEL33_EXTRA_VARIABLE_COLOR_ATTRIB_LOC) in vec4 variable_color;
#endif
#if defined(SHADER_MULTI_DRAW_INDIRECT)
layout(location = SHADER_3DMODEL33_PER_DRAW_DATA_ATTRIB_LOC) in ivec2 per_draw_data;
...
#if !defined (SHADER_MULTI_DRAW_INDIRECT)
// this is like a vulkan push-constant and 
// the only uniform that should be used in 3DMODEL33 shader
layout(location = SHADER_3DMODEL33_ELEM_OFFSET_UNIFORM_LOC) uniform int in_elem_offset;
#endif
...
void main(void)
{
...
#if defined(SHADER_SHADOW_DIR_PASS)
    vec4 view_pos = globals.shadow_light_data.dir_lights_view_matrix[current_dir_light] * world_pos;
#else
    vec4 view_pos = globals.view_matrix * world_pos;
#endif // SHADER_SHADOW_DIR_PASS
	vs_out.object_pos = position;
    vs_out.view_pos = view_pos.xyz;

#if defined(SHADER_FRINGE_NODE_TOP)
    vs_out.fringe_node_top = fringe_node_top;
#endif
#if defined(SHADER_FRINGE_NODE_BOTTOM)
    vs_out.fringe_node_bottom = fringe_node_bottom;
#endif
#if defined(SHADER_USE_BLUR)
	vs_out.velocity_color = velocity_color;
#endif
#if defined(SHADER_LIC_OVERLAY) || defined(SHADER_LIC_SCREEN_SPACE)
	vs_out.vectors = vectors;
#if !defined(SHADER_LIC_SCREEN_SPACE)
	vs_out.vr_Orient = threeAxisDecomp(position - pid_uniforms.lic_data.origFact.xyz);
#endif
#endif
#if defined(SHADER_USE_TEX_COORDS)
	...
	vs_out.nodal_normals = nodal_normals;
#endif
#if defined(SHADER_USE_NODAL_COLOR)
	vs_out.nodal_color = nodal_color;
#endif
#if defined(SHADER_THICK_SHELLS) && defined(SHADER_EXTRA_ELEMENTS)
	vs_out.thickness_values = thickness_values;
	vs_out.thickness_internal_edge_mask = thickness_internal_edge_mask;
#endif
#if defined(SHADER_THICK_SHELLS_GET_NODAL_THICKNESS)
	vs_out.nodal_thickness = nodal_thickness;
#endif
#if defined(SHADER_COLOR_VARIANCE) && defined(SHADER_EXTRA_ELEMENTS)
    vs_out.variable_color = variable_color;
#endif

#if defined(SHADER_MULTI_DRAW_INDIRECT)
	vs_out.part_idx = in_part_idx;
#endif
	vs_out.elem_offset = in_elem_offset;

#if defined(SHADER_SHADOW_DIR_PASS)
    gl_Position = globals.shadow_light_data.dir_lights_projection_matrix[current_dir_light] * view_pos;
#else
