...
void Render(void)
{
	...
	assert(epoxy_has_gl_extension("GL_ARB_sparse_texture_clamp"));
	...
	glGetInternalformativ(GL_TEXTURE_2D, GL_RGB, GL_NUM_VIRTUAL_PAGE_SIZES_ARB,
			      1, &value);
	assert(value == 1);

	glGetInternalformativ(GL_TEXTURE_2D, GL_RGB, GL_VIRTUAL_PAGE_SIZE_X_ARB,
			      1, &value);
	assert(value == 128);
	glGetInternalformativ(GL_TEXTURE_2D, GL_RGB, GL_VIRTUAL_PAGE_SIZE_Y_ARB,
			      1, &value);
	assert(value == 128);
	glGetInternalformativ(GL_TEXTURE_2D, GL_RGB, GL_VIRTUAL_PAGE_SIZE_Z_ARB,
			      1, &value);
	assert(value == 1);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SPARSE_ARB, 1);

	assert(glGetError() == GL_NO_ERROR);

	glTexStorage2D(GL_TEXTURE_2D, 2, GL_RGB8, TARGET_SIZE, TARGET_SIZE);

	assert(glGetError() == GL_NO_ERROR);

	glGetTexParameteriv(GL_TEXTURE_2D, GL_NUM_SPARSE_LEVELS_ARB, &value);
	...
	GLfloat rx = w / TARGET_SIZE;
	GLfloat ry = h / TARGET_SIZE;
        GLfloat tex[] = {
		rx, ry,
		rx, 0,
		0, 0,
