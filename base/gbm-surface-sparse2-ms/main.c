...
void compile(const char *name)
{
	GLint linked;
        GLuint shader;
	assert((shader = LoadShader(name, GL_COMPUTE_SHADER)) != 0);
...
void Render(void)
{
	assert(epoxy_has_gl_extension("GL_ARB_sparse_texture"));
	assert(epoxy_has_gl_extension("GL_ARB_sparse_texture2"));

	GLint value;
	glGetIntegerv(GL_MAX_SPARSE_TEXTURE_SIZE_ARB, &value);
	printf("GL_MAX_SPARSE_TEXTURE_SIZE_ARB=%x\n", value);
	glGetIntegerv(GL_MAX_SPARSE_3D_TEXTURE_SIZE_ARB, &value);
	printf("GL_MAX_SPARSE_3D_TEXTURE_SIZE_ARB=%x\n", value);
	glGetIntegerv(GL_MAX_SPARSE_ARRAY_TEXTURE_LAYERS_ARB, &value);
	printf("GL_MAX_SPARSE_ARRAY_TEXTURE_LAYERS_ARB=%x\n", value);
	glGetIntegerv(GL_SPARSE_TEXTURE_FULL_ARRAY_CUBE_MIPMAPS_ARB, &value);
	printf("GL_SPARSE_TEXTURE_FULL_ARRAY_CUBE_MIPMAPS_ARB=%x\n", value);

	assert(glGetError() == GL_NO_ERROR);

	glGetInternalformativ(GL_TEXTURE_2D_MULTISAMPLE, GL_RGBA8,
	...
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, texin);
	glTexParameteri(GL_TEXTURE_2D_MULTISAMPLE, GL_VIRTUAL_PAGE_SIZE_INDEX_ARB, 0);
	glTexParameteri(GL_TEXTURE_2D_MULTISAMPLE, GL_TEXTURE_SPARSE_ARB, 1);

	assert(glGetError() == GL_NO_ERROR);

	glTexStorage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGBA8,
				  TARGET_SIZE, TARGET_SIZE, true);

	assert(glGetError() == GL_NO_ERROR);

	glGetTexParameteriv(GL_TEXTURE_2D_MULTISAMPLE, GL_NUM_SPARSE_LEVELS_ARB, &value);
	printf("GL_NUM_SPARSE_LEVELS_ARB=%x\n", value);

	assert(glGetError() == GL_NO_ERROR);

	glTexPageCommitmentARB(GL_TEXTURE_2D_MULTISAMPLE, 0, 0, 0, 0, 128, 128, 1, true);

	assert(glGetError() == GL_NO_ERROR);

	glBindImageTexture(0, texin, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);

	assert(glGetError() == GL_NO_ERROR);

	compile("comp.glsl");

	glDispatchCompute(TARGET_SIZE >> 4, TARGET_SIZE >> 4, 1);
	...
	glTextureStorage2D(texout, 1, GL_RGBA8, TARGET_SIZE, TARGET_SIZE);	
	...
	GLubyte *result = calloc(1, TARGET_SIZE * TARGET_SIZE * 4);
	glGetTextureImage(texout, 0, GL_RGBA, GL_UNSIGNED_BYTE,
			  TARGET_SIZE * TARGET_SIZE * 4, result);
	assert(glGetError() == GL_NO_ERROR);
