...
void Render(void)
{
	assert(epoxy_has_gl_extension("GL_ARB_bindless_texture"));
	assert(epoxy_has_gl_extension("GL_ARB_direct_state_access"));
	...
	glTextureStorage2D(texid, 1, GL_RGB8, w, h);	
	glTextureSubImage2D(texid, 0, 0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, data);

	assert(glGetError() == GL_NO_ERROR);

	GLuint64 tex_h = glGetTextureHandleARB(texid);
	glMakeTextureHandleResidentARB(tex_h);
	GLint sample = 0;
	glUniformHandleui64ARB(sample, tex_h);
