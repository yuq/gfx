...
GLuint LoadShader(const char *name, GLenum type)
{
	...
	glShaderBinary(1, &shader, GL_SHADER_BINARY_FORMAT_SPIR_V, buff, size);
...
void InitGLES(void)
{
	...
	assert((vertexShader = LoadShader("vert.spv", GL_VERTEX_SHADER)) != 0);
	assert((fragmentShader = LoadShader("frag.spv", GL_FRAGMENT_SHADER)) != 0);
...
void Render(void)
{
	...
	float ubo_data[] = {0.5, 0, 0};
	glBufferData(GL_UNIFORM_BUFFER, sizeof(ubo_data), ubo_data, GL_STATIC_DRAW);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);

	glBindBufferRange(GL_UNIFORM_BUFFER, 0, ubo, 0, sizeof(ubo_data));

	GLint color_f = 1;
	//GLint color_f = glGetUniformLocation(program, "color_f");
	glUniform1f(color_f, 0);
