...
void Render(int n)
{
	...
	GLint color = glGetUniformLocation(program, "color_v");
	glUniform1f(color, 0.1 * (n + 1));
	...
	char name[32];
	snprintf(name, sizeof(name), "screenshot%d.png", n);
	assert(!writeImage(name, TARGET_W, TARGET_H, result, "hello"));
}
