...
void RenderTargetInit(void)
{
	...
	assert(epoxy_has_egl_extension(display, "EGL_KHR_partial_update"));
...
{
	EGLint age;
	assert(eglQuerySurface(display, surface, EGL_BUFFER_AGE_KHR, &age) == EGL_TRUE);
	printf("buffer %d age %d\n", i, age);
	...
	if (i) {
		...
		assert(eglSetDamageRegionKHR(display, surface, rect, 1));
	}
	...
	glVertexAttribPointer(position, 3, GL_FLOAT, 0, 0, vertex[i]);
	...
	char name[] = "dump0.png";
	name[4] += i;
	assert(!writeImage(name, TARGET_SIZE, TARGET_SIZE, result, "hello"));
}
