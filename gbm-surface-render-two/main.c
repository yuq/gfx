...
void Render(void)
{
	...
	GLint tex0 = glGetAttribLocation(program, "v_texcoord0");
	glEnableVertexAttribArray(tex0);
	glVertexAttribPointer(tex0, 2, GL_FLOAT, 0, 0, texpos);

	GLint tex1 = glGetAttribLocation(program, "v_texcoord1");
	glEnableVertexAttribArray(tex1);
	glVertexAttribPointer(tex1, 2, GL_FLOAT, 0, 0, texpos);
	...
	GLint source_repeat_mode = glGetUniformLocation(program, "source_repeat_mode");
	glUniform1i(source_repeat_mode, 0);

	GLint source_sampler = glGetUniformLocation(program, "source_sampler");
	glUniform1i(source_sampler, 1);

	GLint source_wh = glGetUniformLocation(program, "source_wh");
	glUniform4f(source_wh, 1, 1, 1.0/w, 1.0/h);
