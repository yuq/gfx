...
void rgb2yuv(const char *name)
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, out_tex_y, 0);

	CheckFrameBufferStatus();

	GLuint out_tex_u = 0;
        glGenTextures(1, &out_tex_u);
	glBindTexture(GL_TEXTURE_2D, out_tex_u);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, w / 2, h / 2, 0, GL_RED, GL_UNSIGNED_BYTE, NULL);
	assert(glGetError() == GL_NO_ERROR);

	GLuint out_tex_v = 0;
        glGenTextures(1, &out_tex_v);
	glBindTexture(GL_TEXTURE_2D, out_tex_v);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, w / 2, h / 2, 0, GL_RED, GL_UNSIGNED_BYTE, NULL);
	assert(glGetError() == GL_NO_ERROR);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, out_tex_u, 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, out_tex_v, 0);
	...
	glReadPixels(0, 0, w / 2, h / 2, GL_RED, GL_UNSIGNED_BYTE, data + w * h);
	assert(glGetError() == GL_NO_ERROR);

	glReadBuffer(GL_COLOR_ATTACHMENT1);
	glReadPixels(0, 0, w / 2, h / 2, GL_RED, GL_UNSIGNED_BYTE, data + w * h * 5 / 4);
	assert(glGetError() == GL_NO_ERROR);

	assert(!writeImage("y.png", w, h, w, data, "hello"));
	assert(!writeImage("uv.png", w / 2, h, w / 2, data + h * w, "hello"));
