...
#ifdef ENABLE_DEBUG_LAYERS
	{
		typedef VkResult (*create_callback_func_t)(
			VkInstance instance,
			const VkDebugReportCallbackCreateInfoEXT* pCreateInfo,
			const VkAllocationCallbacks* pAllocator,
			VkDebugReportCallbackEXT* pCallback);

		create_callback_func_t create_callback = (create_callback_func_t)
		...
		assert(create_callback(inst, &info, NULL, &callback) == VK_SUCCESS);
	}
#endif
