...
int load_image(const char *name, int *width, int *height, int *stride)
{
	int w = 0, h = 0;
	void *d = readImage(name, &w, &h);
	printf("read image %s width=%d height=%d\n", name, w, h);
	assert(d);

	struct gbm_bo *bo =
		gbm_bo_create(gbm, w, h, GBM_FORMAT_R8,
			      GBM_BO_USE_LINEAR | GBM_BO_USE_RENDERING);
	assert(bo);

	uint32_t s = 0;
	void *map_data = NULL;
	void *cpu = gbm_bo_map(bo, 0, 0, w, h, GBM_BO_TRANSFER_WRITE, &s, &map_data);
	assert(cpu);
...
{
        int yw = 0, yh = 0, ys = 0;
        int yfd = load_image(ypng, &yw, &yh, &ys);
	assert(yfd >= 0);
	assert(!(yw & 1) && !(yh & 1));

	int uw = 0, uh = 0, us = 0;
        int ufd = load_image(upng, &uw, &uh, &us);
	assert(ufd >= 0);
	assert(yw == uw * 2 && yh == uh * 2);

	int vw = 0, vh = 0, vs = 0;
	int vfd = load_image(vpng, &vw, &vh, &vs);
	assert(vfd >= 0);
	assert(uw == vw && uh == vh);
	...
	EGLint attrib_list[] = {
		...
		EGL_DMA_BUF_PLANE0_PITCH_EXT, ys,
		EGL_DMA_BUF_PLANE1_FD_EXT, ufd,
		EGL_DMA_BUF_PLANE1_OFFSET_EXT, 0,
		EGL_DMA_BUF_PLANE1_PITCH_EXT, us,
		EGL_DMA_BUF_PLANE2_FD_EXT, vfd,
		EGL_DMA_BUF_PLANE2_OFFSET_EXT, 0,
		EGL_DMA_BUF_PLANE2_PITCH_EXT, vs,
